...
# zsh, so we shadow $? into `_projm_ret`.

# ZDOTDIR swap: see zshenv.zsh.
{
  ...
  [ -f "$_projm_user_zdotdir/.zshrc" ] && source "$_projm_user_zdotdir/.zshrc"
...
if [[ -z "$__PROJM_HOOKS_LOADED" ]]; then
  __PROJM_HOOKS_LOADED=1
  ...
  _projm_precmd() {
    local _projm_ret=$?
    printf '\e]133;D;%s\e\\' "$_projm_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_projm_urlencode "$PWD")"
  ...
  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _projm_precmd
    add-zsh-hook preexec _projm_preexec
  fi
