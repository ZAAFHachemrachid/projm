# projm-shell-integration (bashrc)
#
...
if [ -z "$__PROJM_HOOKS_LOADED" ]; then
  ...
  _projm_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_projm_urlencode "$PWD")"
    if [ -z "$__PROJM_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __PROJM_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_projm_precmd:"*) ;;
    *) PROMPT_COMMAND="_projm_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
